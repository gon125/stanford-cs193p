...
struct EmojiArtDocumentView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack {
                PalletChooser(document: document,  chosenPalette: $chosenPalette)
            ...
            GeometryReader { geometry in
                ...
                .onReceive(self.document.$backgroundImage) {
                    if document.steadyStateZoomScale == 1.0 { self.zoomToFit($0, in: geometry.size) } }
            ...
            .navigationBarItems(trailing: Button(action: {
                ...
                Image(systemName: "doc.on.clipboard").imageScale(.large)
                    .alert(isPresented: $explainBackgroundPaste, content: {
                        Alert(title: Text("Paste Background"), message: Text("Copy the URL of an image to clip board and touch this button to make it the background of your document."), dismissButton: .default(Text("OK")))
                    })
            }))
            .zIndex(-1)
        }
